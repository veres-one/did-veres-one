...
const VERES_DID_REGEX = /^(did\:v1)\:(?<mode>test)?(\:)?(?<type>uuid|nym)\:(?<id>.+)/;
...
class VeresOneDidDoc {
  ...
  validateDid({keyPair, env = constants.DEFAULT_ENV} = {}) {
    ...
    const {mode, type, id} = match.groups;
   ...
   * @param [env]
   */
  validateCryptonymDid({keyPair, env = constants.DEFAULT_ENV}) {
    if(!keyPair || !keyPair.publicKey) {
      throw new Error('Public key is required for cryptonym verification');
    }

    const {id} = VERES_DID_REGEX.exec(this.id).groups;
