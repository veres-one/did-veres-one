...
class VeresOneDidDoc {
  constructor(options = {}) {
    ...
    this.doc = options.doc || {
      '@context': [ constants.DID_CONTEXT_URL, constants.VERES_ONE_CONTEXT_URL ]
    };
  ...
  generateKeyObject({key}) {
    return key.publicNode({owner: this.id});
  }
  ...
  async validateDid({env = constants.DEFAULT_ENV} = {}) {
    ...
    if(env !== 'test' && mode === 'test') {
      throw new Error(`Test DID does not match environment/mode "${env}": "${this.id}".`);
    }
