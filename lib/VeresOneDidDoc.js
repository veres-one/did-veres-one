...
class VeresOneDidDoc {
   ...
   */
  keysForProofPurpose(proofPurpose) {
    return this.doc[proofPurpose];
  }

  /**
   * Resolves with an LDKeyPair instance for the specified proof purpose.
   * If no keyId or keyIndex is given, the first available non-revoked
   * key is returned.
   *
   ...
   */
  proofPurposeKey({proofPurpose, keyId, keyIndex = 0}) {
    const keys = this.keysForProofPurpose(proofPurpose);
    if(!keys) {
      throw new Error(`Keys not found for proof purpose "${proofPurpose}".`);
    }

    let keyData;

    if(keyId) {
      keyData = keys.find(k => k.id === keyId);
    } else {
      keyData = keys[keyIndex];
    }
  ...
  async validateCryptonymDid() {
    ...
    const keyPair = await LDKeyPair.from(
      this.proofPurposeKey({proofPurpose: 'capabilityInvocation'}));
  ...
  async validateKeyIds() {
    for(const proofPurpose of Object.values(constants.PROOF_PURPOSES)) {
      const keys = this.keysForProofPurpose(proofPurpose);
