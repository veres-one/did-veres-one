 ...
 * @param {string} [options.capabilityAction = 'write'] - The action being
 *   performed.
 *
...
async function attachProofs(operation, {
  ...
  authenticationKeyPair, authDoc, accelerator, mode, capabilityAction = 'write'
} = {}) {
  if(accelerator) {
  ...
  const invocationTarget = _getInvocationTarget({operation});
