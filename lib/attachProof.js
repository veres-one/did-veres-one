...
const {CapabilityInvocation} = require('@digitalbazaar/zcap');
 ...
 * @param {Function} [options.documentLoader = _documentLoader] - A
 *   documentLoader.
 *
...
async function attachProofs(operation, {
  ...
  operation = await attachInvocationProof({
    operation,
    capability: _getRootZcap({operation}),
    capabilityAction: 'write',
    //FIXME we are not using controller in attachInvocationProof currently
    controller,
    invocationTarget,
    documentLoader,
 ...
 * @param {string} [capabilityAction='write'] - 'read'/'write'.
...
function _getInvocationTarget({operation}) {
  const target = _getOperationTarget({operation});
  if(!target) {
...
function _getRootZcap({operation}) {
  const target = _getOperationTarget({operation});
  if(!target) {
    throw new Error('Unable to determine root zcap for operation.');
  }
