...
async function attachProofs(operation, {
  ...
  // if there is no invocationTarget defined try to find one
  if(!invocationTarget) {
    invocationTarget = _getInvocationTarget({operation});
  }
...
function _getInvocationTarget({operation}) {
  ...
  throw new Error('Unable to determine invocationTarget for record.');
}
