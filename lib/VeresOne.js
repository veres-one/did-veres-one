...
class VeresOne {
  ...
  async attachProofs({operation, options}) {
    ...
    // get private key
    const invokeKeyId = didDocument.doc.capabilityInvocation[0].id;
  ...
  async send(operation, options) {
    ...
    try {
      ...
      console.log('Error in getTargetNode.', e);
      throw e;
    }
  ...
  attachInvocationProof({operation, capability, capabilityAction, creator,
    algorithm, key, privateKeyPem, privateKeyBase58}) {
    ...
    return jsigs.sign(operation, {
      ...
      purpose: new CapabilityInvocation({
        capability: operation.record.id,
