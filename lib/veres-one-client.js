...
class VeresOneClient {
  ...
  async sendToAccelerator(options) {
    ...
    if(authKey && authKey.keyType === 'RsaVerificationKey2018') {
      const exportedKey = await authKey.export();
      ...
      VeresOneClient.signRequestHeaders({
        ...
        keyId: authKey.id,
        key: exportedKey.privateKeyBase58 || exportedKey.privateKeyPem,
        method: 'POST'
      });
    }
  ...
  wrap({didDocument, operationType}) {
    const operation = this.ledger.constructor.wrap(
      {record: didDocument, operationType}
    );
