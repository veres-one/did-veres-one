...
class VeresOneClient {
  ...
  async getTicketServiceProof({operation, ticketService}) {
    let result;
    try {
      // note: ky will add / to the end of the url
      result = await httpClient.post('', {
        prefixUrl: ticketService,
        json: operation, httpsAgent: this.httpsAgent
      });
    } catch(e) {
      const error = new VeresOneClientError('Error retrieving record.',
        'ClientError');
      error.details = {
        url: ticketService, error: e, status: e.status, result
      };
      throw error;
    }
    return result.data;
  ...
  async sendToAccelerator(options) {
    ...
    try {
      // note: ky will add / to the end of the url
      result = await httpClient.post('', {
        prefixUrl: acceleratorUrl,
        json: operation, headers, httpsAgent: this.httpsAgent
      });
    } catch(e) {
      const error = new VeresOneClientError(
        'Error retrieving record.', 'ClientError');
      error.details = {
        url: acceleratorUrl, error: e.data, status: e.status, result
      };
      throw error;
    }
    return result.data;
