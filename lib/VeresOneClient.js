...
const {fromNym} = require('./VeresOneDriver');
...
class VeresOneClient {
   ...
   * @param {WebLedgerClient} [ledger] - Web Ledger Client instance.

   * @param {string} [mode] - One of 'dev'/'test'/'live'.
   * @param {Agent} [httpsAgent] - A NodeJS HTTPS Agent (`https.Agent`)
   *   instance.
   * @param {object} [logger=console] - Logger instance (with .log(), warn() and
   *   error() methods).
   */
   ...
   *
   * @param {string} did - DID uri (possibly with hash fragment)
   * @param {boolean} forceConstruct - Forces deterministic construction of
  ...
  async get({did, forceConstruct = false}) {
    ...
    if(isNym && forceConstruct) {
      didDoc = (await fromNym({did: docUri})).doc;
    } else {
      try {
