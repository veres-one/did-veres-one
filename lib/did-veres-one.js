...
(function(global) {
  ...
  function wrap(api) {
    ...
    api.createCryptonymDid = ({publicKey, encoding, env = 'dev'}) => {
      ...
      cryptonymDid += 'z';
      if(encoding === 'ed25519') {
        // ed25519 cryptonyms are multiformat encoded values,
        // specifically they are: (multicodec 0x30 + ed25519-pub 0xed)
    ...
    api.generate = util.callbackify(async function({
      didType = 'nym', keyType = 'RsaVerificationKey2018',
      passphrase, env = 'dev'
    }) {
      ...
      if(didType === 'nym') {
        ...
        // generate nym using capabilityInvocation app suite
        const did = api.createCryptonymDid({
          publicKey: appSuites.capabilityInvocation.keys.publicKey,
          encoding:
            typeof appSuites.capabilityInvocation.keys.publicKey === 'string' ?
              'ed25519' : 'forge',
          env
        });
     ...
     */
    api.publicDidDocument = util.callbackify(async function(
      {privateDidDocument}) {
      ...
      for(const suite of suites) {
        ...
        for(const item of privateDidDocument[suite]) {
          ...
          for(const key of item.publicKey) {
            ...
            for(const field of [
              '@context', 'publicKeyPem', 'publicKeyBase58'
            ]) {
              if(field in key) {
    ...
    api.attachDelegationProof = util.callbackify(async function(
      // TODO: support `passphrase` for encrypted private key pem or keep
      // decrypt as the responsibility of the caller?
    ...
    api.attachInvocationProof = util.callbackify(async function(
      ...
      // FIXME: use `algorithm` and validate private key, do not switch
      // off of it
      if(privateKeyPem) {
        algorithm = 'RsaSignature2018';
      } else {
