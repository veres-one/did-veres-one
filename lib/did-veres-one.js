...
/**
 * Create a cryptonym DID from a public key with encoding `pem`, `multiformat`,
 * or `forge` (forge is supported privately/internally only).
 */
api.createCryptonymDid = ({publicKey, encoding, env = 'dev'}) => {
  if(!['forge', 'pem', 'multiformat'].includes(encoding)) {
    throw new TypeError(
      '`encoding` must be `pem` or `multiformat`.');
  }
  ...
  if(encoding === 'multiformat') {
    ...
    const cryptonymDid =
      did + 'z' + forge.util.binary.base58.encode(nymBuffer);
