...
class VeresOneDriver {
  ...
  constructor({mode, hostname, httpsAgent, logger, client} = {}) {
    ...
    this.hostname = hostname || VeresOneDriver.defaultHostname(this.mode);
  ...
  async generate({
    ...
    mode = this.mode, invokeKey, authKey, delegateKey, assertionKey
  } = {}) {
    const didDocument = {
    ...
    if(!invokeKey) {
      invokeKey = await cryptoLd.generate({type: keyType});
    }
    // Use the capabilityInvocation key to base the DID URI on
    const did = _generateDid({key: invokeKey, didType, mode});
    if(!invokeKey.controller) {
      invokeKey.controller = did;
      invokeKey.id = `${did}#${invokeKey.fingerprint()}`;
    }
    didDocument.id = did;
    ...
    if(!authKey) {
      authKey = await cryptoLd.generate(keyOptions);
    }

    // Generate a capabilityDelegation key pair
    if(!delegateKey) {
      delegateKey = await cryptoLd.generate(keyOptions);
    }

    // Generate an assertionMethod key pair
    if(!assertionKey) {
      assertionKey = await cryptoLd.generate(keyOptions);
    }
...
function _generateKeyId({did, keyPair}) {
