...
class VeresOneDriver {
   ...
   * @param {CryptoLD} [options.cryptoLd]
   * @param {LDKeyPair class} [options.verificationSuite]
   */
  constructor({
  ...
  /**
   * FIXME: Currently forces offline-only mode (gets unregistered DIDs).
   * (See note on `getFromLedger()`).
   */
  async get({did, url} = {}) {
    return this.getInitial({did, url});
  }

  /**
   * FIXME: Rename back to `get()` when testnet gets updated.
   *
   ...
   * @param {LDKeyPair class} [options.verificationSuite] - Crypto suite to
 ...
 * @param {CryptoLD} cryptoLd - Configured CryptoLD instance.
 * @param {LDKeyPair class} verificationSuite - Crypto suite class to use to
 *   serialize/generate verification keys.
 *
 * @returns {Promise<{didDocument: object, keyPairs: Map,
 *   methodFor: Function}>} Resolves with the generated DID Document.
 */
async function fromNym({did, cryptoLd, verificationSuite} = {}) {
  if(!did) {
